...
Gennode is a **node-powered network for deep health & bio data** — an io.net-style decentralized layer focused on one vertical. Nodes contribute **compute (GPU)** and **storage (disk)**; the network analyzes sensitive biological data privately and at scale.

## Three layers
1. **Compute** — nodes run jobs: DNA & genome analysis, protein folding, drug discovery, medical-imaging and health-AI inference. → [Use cases](/use-cases)
...
3. **Models** — open models today; a purpose-built **Gennode bio/health LLM** served across nodes (distributed inference) over time — **built from scratch if needed**.

## How a job flows
```
Demand (researcher / clinic / app / a person's question)
...
## Privacy (non-negotiable for health data)
